...
st.subheader("✍️ 花几分钟的时间思考一下")

if st.session_state.submitted:
    st.success("✅ 你已经提交过啦，感谢你的分享！")
else:
    with st.form("form"):
        name = st.text_input("我记得你的真名是：")
        mbti = st.text_input("我还不知道你的MBTI是（若未测，凭感觉）：")
        major = st.text_input("你正在学习的专业是：")
        concern = st.text_input("哪些问题不断浮现在你的脑海：")
        city = st.text_input("上大学前往了哪个有趣的城市:")
        want_to_do = st.text_input("你的空闲时光总是用来：")
        want_to_go = st.text_input("你一定要去看看那里的风景：")
        future_topic = st.text_input("你期待在3年后看到当下困扰你的哪一件事的结果：")
